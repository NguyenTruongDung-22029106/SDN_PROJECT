...
def plot_detection_rate_from_dataset():
    ...
    out1 = os.path.join(out_dir, "detection_rate_bar.png")
    ...
    plt.tight_layout()
    out2 = os.path.join(out_dir, "false_alarm_bar.png")
...
def plot_network_traffic_from_runtime():
    ...
    out_dir = os.path.join(base_dir, "output")
    os.makedirs(out_dir, exist_ok=True)
    out = os.path.join(out_dir, "network_traffic_normal_vs_attack.png")
...
def plot_attack_frequency_over_time():
    ...
    out_dir = os.path.join(base_dir, "output")
    os.makedirs(out_dir, exist_ok=True)
    out = os.path.join(out_dir, "ddos_attack_frequency_over_time.png")
    plt.savefig(out, dpi=200)
    plt.close()
    print(f"Đã lưu {out}")
